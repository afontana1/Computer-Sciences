...
class CircularLinkedList(LinkedList):
		...
		if self.head:
			if self.head.value == element:
				...
				if self.head == self.head.next: #if its the only node in the list
					...
					cur.next = self.head.next #point to the node that comes after the head
	...
	def split_list(self):
		...
		while cur and count<mid: #stop when you hit the midway
			...
			prev = cur
			cur = cur.next
		prev.next = self.head #end of the first half, assign the last node's next to be the head
